<x-app-layout>
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">
            {{ $freelancer->name }} - Endereço
        </h2>
    </div>
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <a href="{{ route('freelancers.addresses.index', $freelancer->id) }}"
        ...
        <div class="intro-y col-span-12">
            @if (empty($address->id))
                {!! Form::open([
                    'route' => ['freelancers.addresses.store', $freelancer->id],
                    'method' => 'post',
                    'class' => 'needs-validation',
                ]) !!}
            @else
                {!! Form::model($address, [
                    'route' => ['freelancers.addresses.update', [$freelancer->id, $address->id]],
                    'method' => 'put',
                    'class' => 'needs-validation',
                ]) !!}
            @endif
