<x-app-layout>
    <div class="intro-y flex items-center mt-8">
        <h2 class="text-lg font-medium mr-auto">
            {{ $freelancer->name }} - Documento
        </h2>
    </div>
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <a href="{{ route('freelancers.documents.index', $freelancer->id) }}"
        ...
        <div class="intro-y col-span-12">
            @if (empty($document->id))
                {!! Form::open([
                    'route' => ['freelancers.documents.store', $freelancer->id],
                    'method' => 'post',
            ...
            @else
                {!! Form::model($document, [
                    'route' => ['freelancers.documents.update', [$freelancer->id, $document->id]],
                    'method' => 'put',
                    'class' => 'needs-validation',
                    'files' => 'true',
                ]) !!}
